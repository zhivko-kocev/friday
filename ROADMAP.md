...
- [ ] **Newly-available native surfaces.** Antigravity skills (`~/.gemini/config/skills/`) are now mapped and OpenCode's rules fold into `AGENTS.md` (it has no `rules/` dir). Still to do, each gated on live verification: Codex subagents (TOML `name`/`description`/`developer_instructions`) and Antigravity subagents (JSON `agent.json`) — both need a new markdown→structured-config transform strategy with a pull inverse. Verify-then-map: the Codex skills path (docs say `~/.agents/skills`, current preset writes `~/.codex/skills`) and pi's `APPEND_SYSTEM.md` for core (the audited `earendil-works`/pi.dev pi vs. the `badlogic/pi-mono` the preset targets — confirm they are the same project first).
