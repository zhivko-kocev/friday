...
- **Hooks beyond Claude Code.** The same confirm-first merge-json wiring now extends to every agent that exposes a declarative hook file: **Codex** (`~/.codex/hooks.json`), and — see below — Copilot and Antigravity. Each agent gets a native-dialect `hooks/<agent>/hooks.json` source in the store, and all of them invoke one shared guard script whose deny signal is selected per agent (`exit 2` for Codex; stdout JSON for Copilot/Antigravity). OpenCode and pi are excluded — their hooks are imperative TypeScript plugins, not a declarative file. The non-Claude dialects follow each agent's current docs; verify the guard blocks on a live install (see ROADMAP).
- **Subagent format conversion.** New push-only `md-to-toml` and `md-to-json` rule strategies render Claude-shaped `agents/*.md` into **Codex** TOML subagents (`~/.codex/agents/<name>.toml`) and **Antigravity** JSON subagents — name/description from the frontmatter, the body as the instructions field. Like `concatenate` they don't pull back (the dropped frontmatter is lossy in reverse). TOML is emitted directly, no new dependency. The Antigravity target path and `agent.json` schema are unverified (its docs are unreadable) — verify on a live install before relying on it (see ROADMAP).
